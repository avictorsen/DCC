...
#            files = drivelogin.files().list(fields="nextPageToken, files(name,id)").execute()
                    ...
                    if header == "flowcell_details":
                        ...
                        for i in cell:
                            ...
                            if pair[0] == 'machine':
                                ...
                                elif pair[1] == '@HWI-ST665' or pair[1] == '@HWI-EAS279' or pair[1] == '@HWI-EAS146' or pair[1] == '@HWI-EAS137R' or pair[1] == '@HWUSI-EAS552R' or pair[1] == '@HWI-EAS279' or pair[1] == '@HWI-EAS146':
                ...
                if header == "path_to_file":
                    ...
                    if (temp[1] == '.bam'):
                        ...
                        if proggen == 'dm3':
                            ...
                            os.system("rsync -vPc avictorsen@sullivan.opensciencedatacloud.org:/glusterfs/data/modencode/WhiteLab_UniformProcessing/dm/processed/dm3/old_data/" + path + " ./temp/")
                            file = path.split('/')
                            assembly = 'dm3'
                            path = file[1].rstrip()
                        elif proggen == 'dm6':
                            os.system("rsync -vPc avictorsen@sullivan.opensciencedatacloud.org:/glusterfs/data/modencode/WhiteLab_UniformProcessing/dm/processed/dm6/old_data/" + path + " ./temp/")
...
#            data['replicate'] = d['rep']
    ...
    ############### post ###############
        ...
        i = 0
        err_after_4 = True
        test = str('aws s3 cp ' + d['path'] + ' ' + creds['upload_url'])
        print test
        while (i < 4):
            try:
#                subprocess.check_call(['aws', 's3', 'cp', d['path'], creds['upload_url']], env=env, shell=True)
                subprocess.check_call(test, env=env, shell=True)
                err_after_4 = False
                i = 5
            except subprocess.CalledProcessError as e:
